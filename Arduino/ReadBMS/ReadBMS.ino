...
void ProcessData(void){
  ...
  if (totalCurrent > 50000){
    totalCurrent = convertCurrent(totalCurrent);
  }
...
int convertCurrent (int current){
  int realCurrent;
  return realCurrent = ~current + 1;
}
...
void CANsetup (void){
  ...
  {
      Serial.println("The CAN Shield is connected");
  }
  else
  {
      Serial.println("The CAN Shield init fail");
      Serial.println("Reconnecting CAN ...");
      delay(100);
      goto START_INIT;
  }
}
